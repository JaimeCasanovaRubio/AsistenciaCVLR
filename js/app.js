...
function renderTeamsList() {
    ...
    btn.onclick = () => {
        const nameInput = document.getElementById('newTeam');
        const daysSel = document.getElementById('newDays');
        const name = nameInput.value.trim();
        if (!name)
            return; // No crear equipos sin nombre
        // "as any" aqui porque el select devuelve string generico y TrainingDays es mas estricto.
        addTeam({ name, trainingDays: daysSel.value });
...
function renderTeamDetail(teamId) {
    ...
    const currentDate = window._selDate || today;
    ...
    app.querySelectorAll('[data-del-player]').forEach((el) => {
        el.onclick = () => {
            const pid = el.dataset.delPlayer;
...
function renderPlayerDetail(teamId, playerId) {
    const state = getState();
    const team = state.teams.find((t) => t.id === teamId);
