...
// Importamos los tipos. "import type" = solo para comprobar, no genera codigo.
import type { Team, Player, Attendance, DataState } from './types.js';
...
let state: DataState = emptyState();
...
// ------------------------------------------------------------
// 3. ACCIONES (igual que DataAction en DataContext.tsx:40-48)
// ------------------------------------------------------------
// Una "accion" es un papelito que dice QUE ha pasado.
// El reducer lo lee y decide COMO cambia el estado.
// El "| " significa "puede ser cualquiera de estas formas" (union).
// Payload = los datos que trae la accion.
type DataAction =
  | { type: 'ADD_TEAM', payload: Team } // Añadir equipo entero
  | { type: 'DELETE_TEAM', payload: string } // Borrar equipo por id
  | { type: 'ADD_PLAYER', payload: Player } // Añadir jugador
  | { type: 'DELETE_PLAYER', payload: string } // Borrar jugador por id
  | { type: 'ADD_ATTENDANCE', payload: Attendance } // Añadir o SOBREESCRIBIR marca (misma fecha+jugador)
  | { type: 'DELETE_ATTENDANCE', payload: string } // Borrar marca por id
  | { type: 'LOAD_DATA', payload: DataState }; // Cargar todo desde localStorage
...
function dataReducer(prev: DataState, action: DataAction): DataState {
  ...
  switch (action.type) {
    ...
    case 'ADD_ATTENDANCE': {
      ...
      const idx = prev.attendances.findIndex(
        (a) => a.playerId === action.payload.playerId && a.date === action.payload.date
      );
...
function dispatch(action: DataAction): void {
...
export function addTeam(team: Omit<Team, 'id'>): Team {
  const full: Team = { ...team, id: generateId() };
...
export function addPlayer(player: Omit<Player, 'id'>): Player {
  const full: Player = { ...player, id: generateId() };
...
export function setAttendance(playerId: string, date: string, status: Attendance['status']): void {
...
export function getAttendanceForPlayerOnDate(playerId: string, date: string): Attendance | undefined {
